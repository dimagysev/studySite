...
@endif
<form method="post" enctype="multipart/form-data" action="{{ route('admin.sliders.update', ['slider' => $slide->id]) }}">
    @csrf
    @method('put')
    <div class="card">
                    ...
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <textarea name="title" id="title">{!! $slide->title !!} </textarea>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" style="width: 100%" name="text_position" id="text_position" >
                                    <option value="none" {{ $slide->text_position === 'none' ? 'selected' : '' }}>{{__('pincrio.text_position')}}</option>
                                    <option value="left" {{ $slide->text_position === 'left' ? 'selected' : '' }}>{{__('pincrio.text_left')}}</option>
                                    <option value="right" {{ $slide->text_position === 'right' ? 'selected' : '' }} >{{__('pincrio.text_right')}}</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="desc" id="desc">{{ $slide->desc }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
